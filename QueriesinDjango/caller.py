...
def delete_review_by_id(id_searched: int) -> str:
    review = Review.objects.get(id = id_searched)
    review.delete()
...
def filter_authors_by_nationalities(wanted_nationality: str) -> str:
    authors = Author.objects.filter(nationality=wanted_nationality).order_by('first_name', 'last_name')
    # result = []
    # for a in authors:
    #     if a.biography:
    ...
    return '\n'.join(a.biography if a.biography is not None else f"{a.first_name} {a.last_name}" for a in authors)
...
def filter_authors_by_birth_year(from_year: int, until_year: int) -> str:
    authors = Author.objects.filter(birth_date__year__range=(from_year, until_year)).order_by('-birth_date')

    return '\n'.join(f"{str(a.birth_date)}: {a.first_name} {a.last_name}" for a in authors)
...
def change_reviewer_name(existing_reviewer_name: str, new_name: str):
    # Review.objects.filter(reviewer_name=existing_reviewer_name).update(reviewer_name=new_name)
    # return Review.objects.all()
    reviews = Review.objects.filter(reviewer_name=existing_reviewer_name)
    for r in reviews:
        r.reviewer_name = new_name
    Review.objects.bulk_update(reviews, ['reviewer_name'])
    return Review.objects.all()
